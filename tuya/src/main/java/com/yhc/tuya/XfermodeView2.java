...
public class XfermodeView2 extends View {
    public static final String TEMP_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() +
            "/Pictures/Screenshots/Screenshot_20180228-103015.png";
    public static final String TEMP_PATH2 = Environment.getExternalStorageDirectory().getAbsolutePath() +
            "/Pictures/Screenshots/Screenshot_20180228-103015";
    ...
    private Path mPath;
    int FINGER_WIDTH = 40;
    int BLACK_HALF_COLOR = 0x99000000;
    ...
    public XfermodeView2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XfermodeView2(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            canvas.translate(-(rect.left), -(rect.top));
            ...
            canvas.restore();
            File file = new File(TEMP_PATH2 + "-" + i + ".jpg");
    ...
    public void setBitmap(Bitmap bitmap) {
        mBgBitmap = ImageSizeUtil.zoomImage(bitmap, screenWidth, screenWidth * 4 / 3);
//        mBgBitmap = ImageSizeUtil.decodeSampleBitmap(path, screenWidth, screenWidth);
        mFgBitmap = Bitmap.createBitmap(mBgBitmap.getWidth(), mBgBitmap.getHeight(), Bitmap.Config.ARGB_8888);
        mCanvas = new Canvas(mFgBitmap);
    ...
    public void setTouch(@Nullable MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                listener.onEvent(MotionEvent.ACTION_DOWN);
                listPoint.clear();
...
//                clearCanvas();
    ...
    private Bitmap add2Bitmap(Bitmap first, Bitmap second) {
        ...
        int height = Math.max(first.getHeight(), second.getHeight()) + first.getHeight();
