...
import com.lujianye.commentdemo.ui.CommentFragment
...
import kotlinx.android.synthetic.main.model_activity_comment_base.*
 ...
 * Date : 2018/3/8
 */
abstract class BaseCommentActivity : BaseActivity() {
    /**-------------评论START------------*/
    ...
    override fun getLayoutResID(): Int = R.layout.model_activity_comment_base
...
//        KeyboardUtil.attach(this@BaseCommentActivity, panel_root) { isShowing -> toast("Keyboard is $isShowing") }
        ...
        initDown(fl_content_down)
        //初始化底部导航
//        initBottomNavigation()

    }

    private fun initDown(fl_content_down: FrameLayout?) {
        val transaction = supportFragmentManager.beginTransaction()

        transaction.replace(R.id.fl_content_down, CommentFragment())
...
//        list.add(model1)
//        //底部tab
//        val horizontalRecyclerviewAdapter = HorizontalRecyclerviewAdapter(this@BaseCommentActivity, list)
...
//        recyclerview_horizontal.layoutManager = GridLayoutManager(this@BaseCommentActivity, 1, GridLayoutManager.HORIZONTAL, false)
//
//        //创建全局监听
//        val globalOnItemClickManager = GlobalOnItemClickManagerUtils.getInstance(this@BaseCommentActivity)
//
...
//    }
    ...
    private fun initEmotion() {
        //获取屏幕高度
        val screenWidth = DisplayUtils.getScreenWidthPixels(this@BaseCommentActivity)
        // item的间距
        val spacing = DisplayUtils.dp2px(this@BaseCommentActivity, 12f)
        // 动态计算item的宽度和高度
...
//        vp_complate_emotion_layout.layoutParams = params
    ...
    fun createEmotionGridView(emotionNames: List<String>, gvWidth: Int, padding: Int, itemWidth: Int, gvHeight: Int): GridView {
        // 创建GridView
        val gv = GridView(this@BaseCommentActivity)
        //设置点击背景透明
        ...
        // 给GridView设置表情图片
        val adapter = EmotionGridViewAdapter(this@BaseCommentActivity, emotionNames, itemWidth, EmotionUtils.EMOTION_CLASSIC_TYPE)
        gv.adapter = adapter
        //设置全局点击事件
        gv.onItemClickListener = GlobalOnItemClickManagerUtils.getInstance(this@BaseCommentActivity).getOnItemClickListener(EmotionUtils.EMOTION_CLASSIC_TYPE)
        return gv
    }
}